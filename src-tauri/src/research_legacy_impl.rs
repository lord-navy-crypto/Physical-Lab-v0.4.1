...
fn safe_slug(s: &str) -> String {
    ...
    for c in s.trim().chars() {
        ...
        else if c == '-' || c == '_' || c.is_whitespace() { if !out.ends_with('-') { out.push('-'); } }
    }
...
fn workspace_dir(app: &AppHandle, id: &str) -> Result<PathBuf, String> {
    ...
    if !p.exists() { return Err(format!("Workspace not found: {id}")); }
    Ok(p)
}

fn count_files(p: PathBuf) -> usize {
    fs::read_dir(p).ok().map(|it| it.filter_map(Result::ok).filter(|e| e.path().is_file() || e.path().is_dir()).count()).unwrap_or(0)
}
...
fn sha256(path: &Path) -> Option<String> {
    ...
    let out = Command::new("/usr/bin/shasum").args(["-a", "256", path_s.as_str()]).output().ok()?;
    if !out.status.success() { return None; }
    String::from_utf8_lossy(&out.stdout).split_whitespace().next().map(|s| s.to_string())
}

fn open_path(path: &Path) -> Result<(), String> {
    Command::new("/usr/bin/open").arg(path).spawn().map_err(|e| e.to_string())?;
    Ok(())
}
...
pub fn create_workspace(app: AppHandle, name: String) -> Result<WorkspaceSummary, String> {
    ...
    let mut n = 2;
    while root.join(format!("{id}.physlab")).exists() { id = format!("{base}-{n}"); n += 1; }
    let dir = root.join(format!("{id}.physlab"));
    for child in ["datasets", "measurements", "runs", "figures", "exports", "provenance", "pipelines", "campaigns"] {
    ...
    write_json(&dir.join("pipelines/default-measurement-validation.json"), &default_pipeline_value("measurement-validation"))?;
...
fn workspace_summary_from_dir(dir: &Path) -> Result<WorkspaceSummary, String> {
    ...
    Ok(WorkspaceSummary {
        id: p.get("id").and_then(Value::as_str).unwrap_or_else(|| dir.file_name().and_then(|x|x.to_str()).unwrap_or("workspace")).to_string(),
        name: p.get("name").and_then(Value::as_str).unwrap_or("Physical Lab Project").to_string(),
        created_at: p.get("createdAt").and_then(Value::as_str).unwrap_or("").to_string(),
        updated_at: p.get("updatedAt").and_then(Value::as_str).unwrap_or("").to_string(),
...
pub fn list_workspaces(app: AppHandle) -> Result<Vec<WorkspaceSummary>, String> {
    let mut out = vec![];
    for e in fs::read_dir(workspaces_root(&app)?).map_err(|e| e.to_string())?.filter_map(Result::ok) {
        if e.path().join("project.json").exists() {
            if let Ok(s) = workspace_summary_from_dir(&e.path()) { out.push(s); }
        }
    }
...
pub fn record_run_snapshot(app: AppHandle, workspace_id: String, module_id: String, mode: String, parameters_json: String, results_json: String) -> Result<String, String> {
    let dir = workspace_dir(&app, &workspace_id)?;
    let ts = format!("{}-{}",Local::now().format("%Y%m%d-%H%M%S"),Local::now().timestamp_subsec_millis());
    let id = format!("{}-{}", safe_slug(&module_id), ts);
    let run_dir = dir.join("runs").join(&id);
    fs::create_dir_all(&run_dir).map_err(|e| e.to_string())?;
    let parameters: Value = serde_json::from_str(&parameters_json).unwrap_or_else(|_| json!({"raw":parameters_json}));
    let results: Value = serde_json::from_str(&results_json).unwrap_or_else(|_| json!({"raw":results_json}));
    let module_dir = modules_root(&app)?.join(&module_id);
    let source = module_dir.join("source");
    let commit = if source.exists() { command_text_in(&source, "git", &["rev-parse","HEAD"]) } else { None };
    let py = module_dir.join(".venv/bin/python");
    let python = if py.exists() { command_text(py.to_string_lossy().as_ref(), &["--version"]) } else { None };
    let freeze = if py.exists() { command_text(py.to_string_lossy().as_ref(), &["-m","pip","freeze","--all"]) } else { None };
    write_json(&run_dir.join("run.json"), &json!({
...
pub fn import_measurement_dataset(app: AppHandle, workspace_id: String, source_path: String, name: String, quantity: String, unit: String, sensor: String, calibration: String) -> Result<DatasetSummary, String> {
    ...
    if !src.is_file() { return Err(format!("Measurement file does not exist: {}", src.display())); }
    let ext = src.extension().and_then(|x|x.to_str()).unwrap_or("").to_ascii_lowercase();
    if !["csv","tsv","json","h5","hdf5"].contains(&ext.as_str()) { return Err("Supported measurement formats: CSV, TSV, JSON, HDF5".into()); }
    let dir = workspace_dir(&app, &workspace_id)?;
    let ds_id = format!("{}-{}", safe_slug(&name), Local::now().format("%Y%m%d-%H%M%S"));
    ...
    let created = now_iso();
    write_json(&ds_dir.join("metadata.json"), &json!({
    ...
    touch_project(&dir)?;
    Ok(DatasetSummary{id:ds_id,name,quantity,unit,sensor,format:ext,source_file:src.to_string_lossy().to_string(),stored_file:stored.to_string_lossy().to_string(),sha256:hash,created_at:created})
}
    ...
    for e in fs::read_dir(dir).map_err(|e| e.to_string())?.filter_map(Result::ok) {
        ...
        if let Ok(v) = read_json(&meta) {
            out.push(DatasetSummary{
                id:v.get("id").and_then(Value::as_str).unwrap_or("").to_string(),name:v.get("name").and_then(Value::as_str).unwrap_or("").to_string(),
                quantity:v.get("quantity").and_then(Value::as_str).unwrap_or("").to_string(),unit:v.get("unit").and_then(Value::as_str).unwrap_or("").to_string(),
                sensor:v.get("sensor").and_then(Value::as_str).unwrap_or("").to_string(),format:v.get("format").and_then(Value::as_str).unwrap_or("").to_string(),
                source_file:v.get("sourceFile").and_then(Value::as_str).unwrap_or("").to_string(),stored_file:v.get("storedFile").and_then(Value::as_str).unwrap_or("").to_string(),
                sha256:v.get("sha256").and_then(Value::as_str).map(str::to_string),created_at:v.get("createdAt").and_then(Value::as_str).unwrap_or("").to_string()
            });
        }
    }
...
pub fn list_serial_devices() -> Result<Vec<String>, String> {
    let mut out = vec![];
    for e in fs::read_dir("/dev").map_err(|e| e.to_string())?.filter_map(Result::ok) {
        let name = e.file_name().to_string_lossy().to_string();
        if name.starts_with("cu.") || name.starts_with("tty.") { out.push(format!("/dev/{name}")); }
    }
    out.sort();
    Ok(out)
}
...
pub fn capture_serial_measurement(app: AppHandle, workspace_id: String, device: String, baud: u32, seconds: u64, name: String, quantity: String, unit: String, sensor: String) -> Result<DatasetSummary, String> {
    if !device.starts_with("/dev/cu.") && !device.starts_with("/dev/tty.") { return Err("Only macOS serial devices under /dev/cu.* or /dev/tty.* are accepted.".into()); }
    if !Path::new(&device).exists() { return Err("Serial device not found.".into()); }
    let secs = seconds.clamp(1, 300);
    let baud_s=baud.to_string();
    let _ = Command::new("/bin/stty").args(["-f", device.as_str(), baud_s.as_str(), "raw", "-echo"]).status();
...
os.close(fd)
"#;
    let py = command_text("/usr/bin/which", &["python3"]).filter(|s| !s.is_empty()).unwrap_or_else(|| "python3".into());
    let secs_s=secs.to_string(); let tmp_s=tmp.to_string_lossy().to_string();
    let status = Command::new(py.trim()).args(["-c", code, device.as_str(), secs_s.as_str(), tmp_s.as_str()]).status().map_err(|e| e.to_string())?;
    if !status.success() { return Err("Serial capture failed. Check device permissions and baud rate.".into()); }
    let result = import_measurement_dataset(app, workspace_id, tmp.to_string_lossy().to_string(), name, quantity, unit, sensor, format!("Serial capture at {baud} baud for {secs}s"));
...
fn parse_csv_numeric(path: &Path) -> Result<(Vec<String>, Vec<Vec<Option<f64>>>), String> {
    ...
    let header = lines.next().ok_or("Dataset is empty")?.map_err(|e| e.to_string())?;
    let sep = if header.contains('\t') { '\t' } else { ',' };
    let headers: Vec<String> = header.split(sep).map(|s|s.trim().trim_matches('"').to_string()).collect();
...
pub fn analyze_dataset(app: AppHandle, workspace_id: String, dataset_id: String) -> Result<Vec<ColumnStats>, String> {
    let meta = read_json(&workspace_dir(&app,&workspace_id)?.join("datasets").join(&dataset_id).join("metadata.json"))?;
    let format = meta.get("format").and_then(Value::as_str).unwrap_or("");
    if format != "csv" && format != "tsv" { return Err("Numeric Result Center currently analyzes CSV/TSV directly; JSON/HDF5 remain preserved for Lab-specific readers.".into()); }
    let path = PathBuf::from(meta.get("storedFile").and_then(Value::as_str).ok_or("Dataset path missing")?);
    ...
    for (h,c) in headers.into_iter().zip(cols.into_iter()) {
        ...
        let n=v.len(); let mean=v.iter().sum::<f64>()/n as f64; let var=if n>1{v.iter().map(|x|(x-mean)*(x-mean)).sum::<f64>()/(n-1) as f64}else{0.0};
        let sd=var.sqrt(); let half=if n>1{1.96*sd/(n as f64).sqrt()}else{0.0};
        out.push(ColumnStats{column:h,n,mean,std_dev:sd,ci95_low:mean-half,ci95_high:mean+half,min:v.iter().cloned().fold(f64::INFINITY,f64::min),max:v.iter().cloned().fold(f64::NEG_INFINITY,f64::max)});
    }
    Ok(out)
}
...
pub fn validate_dataset_columns(app: AppHandle, workspace_id:String, dataset_id:String, observed_column:String, reference_column:String) -> Result<ValidationResult,String>{
    ...
    let path=PathBuf::from(meta.get("storedFile").and_then(Value::as_str).ok_or("Dataset path missing")?);
    let (headers,cols)=parse_csv_numeric(&path)?;
    let oi=headers.iter().position(|h|h==&observed_column).ok_or("Observed column not found")?;
    let ri=headers.iter().position(|h|h==&reference_column).ok_or("Reference column not found")?;
    let pairs:Vec<(f64,f64)>=cols[oi].iter().zip(cols[ri].iter()).filter_map(|(a,b)|Some(((*a)?,(*b)?))).filter(|(a,b)|a.is_finite()&&b.is_finite()).collect();
    if pairs.len()<2{return Err("Need at least two finite observed/reference pairs.".into())}
    let n=pairs.len(); let mae=pairs.iter().map(|(a,b)|(a-b).abs()).sum::<f64>()/n as f64; let mse=pairs.iter().map(|(a,b)|(a-b)*(a-b)).sum::<f64>()/n as f64; let rmse=mse.sqrt();
    let max_abs_error=pairs.iter().map(|(a,b)|(a-b).abs()).fold(0.0,f64::max); let ref_mean=pairs.iter().map(|(_,b)|*b).sum::<f64>()/n as f64;
    let ref_scale=pairs.iter().map(|(_,b)|b.abs()).sum::<f64>()/n as f64; let relative_rmse=if ref_scale>1e-15{Some(rmse/ref_scale)}else{None};
    let ss_tot=pairs.iter().map(|(_,b)|(b-ref_mean)*(b-ref_mean)).sum::<f64>(); let ss_res=pairs.iter().map(|(a,b)|(a-b)*(a-b)).sum::<f64>(); let r2=if ss_tot>1e-30{Some(1.0-ss_res/ss_tot)}else{None};
    let ratio=relative_rmse.unwrap_or(rmse); let agreement=if ratio<0.01{"Strong"}else if ratio<0.05{"Good"}else if ratio<0.15{"Moderate"}else{"Weak"}.to_string();
...
fn command_text_in(dir:&Path,program:&str,args:&[&str])->Option<String>{
    let out=Command::new(program).current_dir(dir).args(args).output().ok()?; if !out.status.success(){return None} Some(String::from_utf8_lossy(&out.stdout).trim().to_string())
}
...
fn default_pipeline_value(kind:&str)->Value{
    match kind{
        "accelerator-measurement"=>json!({"schema":"physical-lab-pipeline-v1","name":"Measured Field → RADIA → Radiation","steps":[
        ...
        "oscillation-modal"=>json!({"schema":"physical-lab-pipeline-v1","name":"Oscillation → Chrono::Modal Comparison","steps":[
        ...
        "atomistic-magnetism"=>json!({"schema":"physical-lab-pipeline-v1","name":"Material Inputs → VAMPIRE → Result Import","steps":[
        ...
        _=>json!({"schema":"physical-lab-pipeline-v1","name":"Measurement → Validation","steps":[{"id":"measurement","type":"dataset","label":"Measurement dataset","status":"input"},{"id":"analysis","type":"results","label":"Statistics / uncertainty","status":"ready"},{"id":"validation","type":"validation","label":"Observed vs reference","status":"ready"}]})
    }
}

#[tauri::command]
pub fn pipeline_templates()->Vec<Value>{vec![default_pipeline_value("accelerator-measurement"),default_pipeline_value("oscillation-modal"),default_pipeline_value("atomistic-magnetism"),default_pipeline_value("measurement-validation")]}

#[tauri::command]
pub fn save_pipeline(app:AppHandle,workspace_id:String,kind:String)->Result<String,String>{
    let dir=workspace_dir(&app,&workspace_id)?; let v=default_pipeline_value(&kind); let id=format!("{}-{}",safe_slug(&kind),Local::now().format("%Y%m%d-%H%M%S")); write_json(&dir.join("pipelines").join(format!("{id}.json")),&v)?; touch_project(&dir)?; Ok(id)
}
...
pub fn create_campaign(app:AppHandle,workspace_id:String,module_id:String,parameter:String,start:f64,stop:f64,points:u32,max_parallel:u32)->Result<String,String>{
    if points<2||points>10000{return Err("Campaign points must be between 2 and 10000.".into())} let dir=workspace_dir(&app,&workspace_id)?; let id=format!("{}-campaign-{}",safe_slug(&module_id),Local::now().format("%Y%m%d-%H%M%S"));
    let vals:Vec<f64>=(0..points).map(|i|start+(stop-start)*(i as f64)/((points-1) as f64)).collect(); let jobs:Vec<Value>=vals.iter().enumerate().map(|(i,v)|json!({"id":format!("run-{:04}",i+1),"parameter":&parameter,"value":v,"status":"queued"})).collect();
    write_json(&dir.join("campaigns").join(format!("{id}.json")),&json!({"schema":"physical-lab-campaign-v1","id":&id,"createdAt":now_iso(),"moduleId":module_id,"parameter":parameter,"start":start,"stop":stop,"points":points,"maxParallel":max_parallel.clamp(1,8),"queueState":"ready","jobs":jobs,"execution":"queue-and-handoff","note":"Campaign parameters are persisted reproducibly. Current Lab UIs remain solver owners until module-specific parameter adapters are added."}))?; touch_project(&dir)?; Ok(id)
}
...
pub fn export_reproducibility_package(app:AppHandle,workspace_id:String)->Result<String,String>{
    let dir=workspace_dir(&app,&workspace_id)?; let prov=dir.join("provenance"); fs::create_dir_all(&prov).map_err(|e|e.to_string())?;
    let specs=lab_specs()?; let mut mods=vec![]; let root=modules_root(&app)?;
    for m in specs{let md=root.join(&m.id); let src=md.join("source"); let py=md.join(".venv/bin/python"); mods.push(json!({"id":m.id,"name":m.name,"sourceCommit":if src.exists(){command_text_in(&src,"git",&["rev-parse","HEAD"])}else{None},"python":if py.exists(){command_text(py.to_string_lossy().as_ref(),&["--version"])}else{None},"pipFreeze":if py.exists(){command_text(py.to_string_lossy().as_ref(),&["-m","pip","freeze","--all"])}else{None}}));}
    write_json(&prov.join("environment.json"),&json!({"createdAt":now_iso(),"machine":{"arch":command_text("uname",&["-m"]),"os":command_text("sw_vers",&["-productVersion"]),"build":command_text("sw_vers",&["-buildVersion"])},"modules":mods}))?;
    let export_dir=app_root(&app)?.join("exports"); fs::create_dir_all(&export_dir).map_err(|e|e.to_string())?; let out=export_dir.join(format!("{}-reproducible-{}.zip",safe_slug(&workspace_id),Local::now().format("%Y%m%d-%H%M%S")));
    let dir_s=dir.to_string_lossy().to_string(); let out_s=out.to_string_lossy().to_string(); let base_name=dir.file_name().and_then(|x|x.to_str()).unwrap_or("workspace").to_string();
    let status=Command::new("/usr/bin/ditto").args(["-c","-k","--sequesterRsrc","--keepParent",dir_s.as_str(),out_s.as_str()]).status().or_else(|_|Command::new("/usr/bin/zip").current_dir(dir.parent().unwrap_or(&dir)).args(["-r",out_s.as_str(),base_name.as_str()]).status()).map_err(|e|e.to_string())?;
    if !status.success(){return Err("Reproducibility archive creation failed".into())} Ok(out.to_string_lossy().to_string())
}

#[tauri::command]
pub fn list_run_snapshots(app:AppHandle,workspace_id:String)->Result<Vec<Value>,String>{
    let runs=workspace_dir(&app,&workspace_id)?.join("runs"); let mut out=vec![];
    for e in fs::read_dir(runs).map_err(|e|e.to_string())?.filter_map(Result::ok){
        let p=e.path().join("run.json"); if let Ok(v)=read_json(&p){out.push(v)}
    }
    out.sort_by(|a,b|b.get("createdAt").and_then(Value::as_str).unwrap_or("").cmp(a.get("createdAt").and_then(Value::as_str).unwrap_or("")));
    Ok(out)
}

fn flatten_json(prefix:&str,v:&Value,out:&mut HashMap<String,String>){
    match v{
        Value::Object(m)=>for(k,val)in m{let p=if prefix.is_empty(){k.clone()}else{format!("{prefix}.{k}")};flatten_json(&p,val,out)},
        Value::Array(a)=>{out.insert(prefix.into(),format!("[{} items]",a.len()));},
        _=>{out.insert(prefix.into(),v.to_string());}
    }
}

#[tauri::command]
pub fn compare_run_snapshots(app:AppHandle,workspace_id:String,run_a:String,run_b:String)->Result<Value,String>{
    let base=workspace_dir(&app,&workspace_id)?.join("runs"); let a=read_json(&base.join(&run_a).join("run.json"))?; let b=read_json(&base.join(&run_b).join("run.json"))?;
    let mut fa=HashMap::new();let mut fb=HashMap::new();flatten_json("",&a,&mut fa);flatten_json("",&b,&mut fb);
    let mut keys:Vec<String>=fa.keys().chain(fb.keys()).cloned().collect();keys.sort();keys.dedup();
    let diffs:Vec<Value>=keys.into_iter().filter_map(|k|{let av=fa.get(&k);let bv=fb.get(&k);if av==bv{None}else{Some(json!({"field":k,"a":av,"b":bv}))}}).collect();
    let count=diffs.len();
...
pub fn list_campaigns(app:AppHandle,workspace_id:String)->Result<Vec<Value>,String>{
    let dir=workspace_dir(&app,&workspace_id)?.join("campaigns"); let mut out=vec![];
    for e in fs::read_dir(dir).map_err(|e|e.to_string())?.filter_map(Result::ok){if e.path().extension().and_then(|x|x.to_str())==Some("json"){if let Ok(v)=read_json(&e.path()){out.push(v)}}}
    out.sort_by(|a,b|b.get("createdAt").and_then(Value::as_str).unwrap_or("").cmp(a.get("createdAt").and_then(Value::as_str).unwrap_or(""))); Ok(out)
}

#[tauri::command]
pub fn campaign_action(app:AppHandle,workspace_id:String,campaign_id:String,action:String)->Result<Value,String>{
    let path=workspace_dir(&app,&workspace_id)?.join("campaigns").join(format!("{campaign_id}.json")); let mut v=read_json(&path)?; let act=action.as_str();
    match act{
        "pause"=>v["queueState"]=Value::String("paused".into()),
        "resume"=>v["queueState"]=Value::String("ready".into()),
        "retry-failed"=>if let Some(jobs)=v.get_mut("jobs").and_then(Value::as_array_mut){for j in jobs{if j.get("status").and_then(Value::as_str)==Some("failed"){j["status"]=Value::String("queued".into())}}},
        "reset"=>if let Some(jobs)=v.get_mut("jobs").and_then(Value::as_array_mut){for j in jobs{j["status"]=Value::String("queued".into())}},
        _=>return Err("Supported campaign actions: pause, resume, retry-failed, reset".into())
    }
    v["updatedAt"]=Value::String(now_iso()); write_json(&path,&v)?; Ok(v)
}
