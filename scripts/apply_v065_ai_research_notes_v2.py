#!/usr/bin/env python3
"""Robust one-time patch: opt-in .physlab provenance for Local AI exchanges."""
...
'''
    ...
    text = replace_once(text, render_anchor, helpers + render_anchor, "research note helpers")

if "__physical_lab_ai_exchange_" not in text:
    assignment = '                st.session_state[f"pl_local_ai_answer_{profile}"] = answer'
    ...
    text = replace_once(text, assignment, replacement, "AI exchange snapshot")

if "Save this exchange to .physlab provenance" not in text:
    caption = '            st.caption("AI explanation is advisory. Check units, model assumptions, measured data, uncertainty and solver validation before drawing scientific conclusions.")'
    save_ui = caption + r'''
...
'''
    text = replace_once(text, caption, save_ui, "AI advisory caption")
...
print("v0.65 AI research provenance patch v2: APPLIED")
